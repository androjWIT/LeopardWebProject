...
void ADMIN::addCourse(){
    ...
    std::cin >> daysofweek;
    std::cout <<  "Enter the semester the couse will take place in: \n";
    std::cin >> sem;
...
void ADMIN::addInstructor(){
    std::string ID, FName, Lname, title, hireYear, dept, email;
...
void ADMIN::linkInstructor(){
    ...
    std::string sql = "UPDATE COURSE SET InstructorID = ? WHERE CRN = ?";
    sqlite3_stmt* stmt;
...
void ADMIN::unlinkInstructor(){
    ...
    std::string sql = "UPDATE COURSE SET InstructorID = NULL WHERE CRN = ?";
    sqlite3_stmt* stmt;
