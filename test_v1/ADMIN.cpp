...
void ADMIN::menu(){
    ...
    do{
        ...
        std::cout << "4. Delete course from course \n";
...
// testing functions
// Non-interactive helper: adds a course without user input
bool ADMIN::addCourse_test(const std::string& crn, const std::string& title, const std::string& dep,
                            const std::string& time, const std::string& days,
                            const std::string& semester, const std::string& year,
                            const std::string& instructor) {
    std::string sql = "INSERT INTO COURSE (CRN, TITLE, DEPARTMENT, TIME, DAYS_OF_WEEK, SEMESTER, YEAR, INSTRUCTOR) "
                      "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    sqlite3_stmt* stmt;

    int rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "Prepare failed in addCourseDirect: " << sqlite3_errmsg(db) << "\n";
        return false;
    }
    ...
    if (!success) {
        std::cerr << "Insert failed in addCourseDirect: " << sqlite3_errmsg(db) << "\n";
    }
...
// Non-interactive helper: search course by CRN
bool ADMIN::searchCourseByCRN_test(const std::string& crn, std::string& outTitle) {
    std::string sql = "SELECT TITLE FROM COURSE WHERE CRN = ?";
    sqlite3_stmt* stmt;

    int rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "Prepare failed in searchCourseByCRN: " << sqlite3_errmsg(db) << "\n";
        return false;
    }
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        outTitle = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        found = true;
    }
...
bool ADMIN::searchCourseByDep_test(const std::string& dep, std::string& outdep) {
    std::string sql = "SELECT CRN FROM COURSE WHERE DEPARTMENT = ?";
    sqlite3_stmt* stmt;

    int rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "Prepare failed in searchCourseByDepartment: " << sqlite3_errmsg(db) << "\n";
        return false;
    }

    sqlite3_bind_text(stmt, 1, dep.c_str(), -1, SQLITE_TRANSIENT);

    bool found = false;
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        outdep = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        found = true;
    }
...
// Non-interactive helper: search course by Title
bool ADMIN::searchCourseByTitle_test(const std::string& title, std::string& outCRN) {
    std::string sql = "SELECT CRN FROM COURSE WHERE TITLE = ?";
    sqlite3_stmt* stmt;

    int rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "Prepare failed in searchCourseByTitle: " << sqlite3_errmsg(db) << "\n";
        return false;
    }
    ...
    bool found = false;
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        outCRN = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
        found = true;
    }
...
bool ADMIN::removeCourseByCRN_test(const std::string& crn)
{
      std::string sql = "DELETE FROM COURSE WHERE CRN = ?";
    sqlite3_stmt* stmt;
    ...
    int rc = sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        std::cerr << "Prepare failed in removeCourseByCRN: " << sqlite3_errmsg(db) << "\n";
        return false;
    }
    ...
    if (!success) {
        std::cerr << "Delete failed in removeCourseByCRN: " << sqlite3_errmsg(db) << "\n";
    }
